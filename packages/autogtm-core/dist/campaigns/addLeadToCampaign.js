...
import { getSupabaseClient, incrementCampaignLeadCount, markLeadRouted, markLeadSkipped, } from '../db/autogtmDbCalls';
...
const AUTO_REGENERATE_FEEDBACK = "This draft may be stale or templated. Rewrite the sequence so the opening hook references a specific detail from this lead's bio, expertise, or audience — nothing generic. Keep copy concise and personal; avoid templated phrasing in subjects.";
/**
...
export async function regenerateAndSaveDraftEmails(params) {
    ...
    const { data: latestVersionRows } = await supabase
        ...
        .limit(1);
    const nextVersion = (latestVersionRows?.[0]?.version_number || 0) + 1;
    await supabase.from('campaign_email_versions').insert(emails.map((e) => ({
...
export async function addLeadToCampaignCore(params) {
    ...
    if (softFail) {
        if (!campaign.is_accepting_leads) {
            if (markSkipped)
                await markLeadSkipped(leadId, 'campaign_not_accepting');
            return { ok: false, reason: 'campaign_not_accepting' };
        }
        if ((campaign.leads_count || 0) >= (campaign.max_leads || 500)) {
            if (markSkipped)
                await markLeadSkipped(leadId, 'campaign_full');
...
//# sourceMappingURL=addLeadToCampaign.js.map