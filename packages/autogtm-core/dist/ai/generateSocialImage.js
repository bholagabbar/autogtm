...
function getVideoModel(model) {
    if (model)
        return model;
    const fromEnv = (process.env.OPENROUTER_VIDEO_MODEL || 'google/veo-3.1-fast');
    if (fromEnv === 'google/veo-3.1-fast' || fromEnv === 'bytedance/seedance-2.0-fast')
        return fromEnv;
    return 'google/veo-3.1-fast';
}
async function startOpenRouterVideoGeneration(prompt, model) {
...
async function pollOpenRouterVideoResult(generationId, pollingUrl, timeoutMs = OPENROUTER_VIDEO_TIMEOUT_MS) {
    const apiKey = getOpenRouterApiKey();
...
export async function ensureSocialImageBucket(bucketName = process.env.SUPABASE_STORAGE_BUCKET_SOCIAL || DEFAULT_BUCKET) {
...
export async function generateSocialImage(params) {
    ...
    if (mode === 'video') {
        ...
        if (!mediaRes.ok)
            throw new Error(`Failed downloading generated video: ${mediaRes.status}`);
...
//# sourceMappingURL=generateSocialImage.js.map